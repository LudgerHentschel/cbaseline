...
BASE = "https://ludgerhentschel.github.io/cbaseline/"
...
def main():
    ...
    for name in ("index", "getting-started", "backgrounds", "api", "diagnostics"):
        assert BASE + name + ".html" in locations, f"Guide omitted from sitemap: {name}"
    api = (build / "api.html").read_text()
    assert 'id="cbaseline.background"' in api, "API reference was not expanded"
    quickstart = (build / "getting-started.html").read_text()
    assert "predictions" in quickstart and "assert_allclose" in quickstart, "Quickstart was not expanded"
